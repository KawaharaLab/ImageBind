...
# 各動画をIMUに合わせて切り抜く
def cut_videos_to_imu(imu_dir: str, video_dir: str, output_dir: str, codec_copy: bool = True):
...
def encode_videos():
    ...
    for mp4_path in sorted(glob.glob(os.path.join(video_dir, "*.mp4"))):
        ...
        try:
            video = load_and_transform_video_data([mp4_path], device=device, clips_per_video=10)
